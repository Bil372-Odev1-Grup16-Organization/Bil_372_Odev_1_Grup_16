...
if (isset($_GET['M_SYSCODE'])) {
    ...
    if (isset($_GET['confirm']) and isset($_GET['option'])) {
        if ($_GET['confirm'] == 'yes') {
            $stmt = $pdo->prepare(
                'SELECT M_ABSTRACT, M_PARENTCODE,M_CODE FROM PRODUCT WHERE M_SYSCODE = ?'
            );
            $stmt->execute([$_GET['M_SYSCODE']]);
            $product = $stmt->fetch();
            ...
            if ($_GET['option'] == 'cascade') {
                ...
                if ($product['M_ABSTRACT'] >= 1) {
                    //abstract
                    $stmt = $pdo->prepare(
                        'UPDATE PRODUCT SET IS_ACTIVE=0 WHERE M_SYSCODE = ?'
                    );
                    $stmt->execute([$_GET['M_SYSCODE']]);
                } else {
                    //not abstract
                    ...
                    $stmt = $pdo->prepare(
                        'UPDATE PRODUCT SET IS_ACTIVE=0 WHERE M_PARENTCODE =  ?'
                    );
                ...
                $stmt = $pdo->prepare(
                    'UPDATE PRODUCT SET M_PARENTCODE = ? WHERE M_PARENTCODE =  ?'
                );
